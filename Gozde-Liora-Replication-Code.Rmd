...
somers2(model_1$fitted.values, model_1$y)
pred <- model_1$fitted.values # Create vector with predicted values from the model
actual <- model_1$y # Create vector with actual values for vote intention for extreme right parties
dev.off()
plot_model1 <- separationplot(pred = pred, actual = actual, type = "rect", line = T, show.expected = F) 
...
actual <- model_2$y
dev.off()
plot_model2 <- separationplot(pred = pred, actual = actual, type = "rect", line = T, show.expected = F)
...
```{r}
model_3 <- glm(ext_vote ~ dispro_elect + federalism + z_asylumseeker + z_unemp + z_asylumseeker:z_unemp + z_replacement + z_replacement:z_unemp + z_replacement:z_asylumseeker + max_er + salience_mean_c + var_c + var_c:salience_mean_c + country - 1, data = data)

pred <- model_3$fitted.values
actual <- model_3$y
dev.off()
plot_model3 <- separationplot(pred = pred, actual = actual, type = "rect", line = T, show.expected = F)
```
